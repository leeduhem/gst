...
AC_DEFUN([GST_PACKAGE_ENABLE], [
  ...
  cat >> pkgrules.tmp << \EOF
-include $(top_builddir)/_GST_PKG_MK
all-local: $(top_builddir)/_GST_PKG_STAMP
$1.star: _GST_PKG_XML $(top_builddir)/_GST_PKG_STAMP _GST_PKG_DEPENDENCIES
...
$(top_builddir)/_GST_PKG_MK: _GST_PKG_XML_IN gst-tool$(EXEEXT) gst.im
	$(mkdir_p) $(top_builddir)/_GST_PKG_DIR
	(echo '$1_FILES = \'; \
	  $(GST_[]PACKAGE) --srcdir=$(srcdir) --vpath --list-files $1 $< | sort | \
	    tr -d \\r | tr \\n " "; \
	echo; \
	echo '$$($1_FILES):'; \
	echo '$$(top_builddir)/_GST_PKG_STAMP: $$($1_FILES)'; \
	echo '	touch $$(top_builddir)/_GST_PKG_STAMP') > $(top_builddir)/_GST_PKG_MK
EOF
